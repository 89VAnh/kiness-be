...
export class GrowthStoryService {
    ...
    async getDetailGrowthStory(growth_story_id: number): Promise<any> {
        return await this.growthStoryRepository.getDetailGrowthStory(growth_story_id);
    }

    async getDetailClientGrowthStory(growth_story_id: number): Promise<any> {
        return await this.growthStoryRepository.getDetailClientGrowthStory(growth_story_id);
    }

    async searchGrowthStories(search_gs_model: SearchGrowthStoryModel): Promise<any> {
        return await this.growthStoryRepository.searchGrowthStories(search_gs_model);
    }

    async searchClientGrowthStories(search_client_gs: SearchClientGrowthStoryModel): Promise<any> {
        return await this.growthStoryRepository.searchClientGrowthStories(search_client_gs);
    }
