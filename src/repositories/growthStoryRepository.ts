...
export class GrowthStoryRepository {
  ...
  async createGrowthStory(gs: GrowthStoryModel): Promise<any> {
    try {
      const sql =
        "CALL InsertGrowthStory(?, ?, ?, ?, ?, ?, ?, @err_code, @err_msg)";
  ...
  async updateGrowthStory(gs: GrowthStoryModel): Promise<any> {
    try {
      const sql =
        "CALL UpdateGrowthStory(?, ?, ?, ?, ?, ?, ?, ?, @err_code, @err_msg)";
      await this.db.query(sql, [
        gs.growth_story_id,
        gs.title,
        gs.content,
  ...
  async updateViewCountGrowthStory(growth_story_id: number): Promise<any> {
    try {
      ...
      await this.db.query(sql, [growth_story_id]);
    } catch (error: any) {
      throw new DatabaseError(error.message);
    }
  }
  ...
  async getDetailGrowthStory(growth_story_id: number): Promise<any> {
    try {
      ...
      const [results] = await this.db.query(sql, [growth_story_id]);
      return results[0];
    } catch (error: any) {
      throw new DatabaseError(error.message);
    }
  ...
  async searchGrowthStories(
    ...
    try {
      ...
      const [results] = await this.db.query(sql, [
        search_gs_model.page_index,
        search_gs_model.page_size,
  ...
  async searchClientGrowthStories(
    ...
    try {
      const sql =
        "CALL SearchClientGrowthStories(?, ?, ?, @err_code, @err_msg)";
      const [results] = await this.db.query(sql, [
        search_client_gs.page_index,
        search_client_gs.page_size,
