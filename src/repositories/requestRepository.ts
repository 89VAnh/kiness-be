...
export class RequestRepository {
  ...
  async createRequest(req: RequestModel): Promise<any> {
    try {
      const sql =
        "CALL InsertRequest(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, @err_code, @err_msg)";
  ...
  async updateRequest(req: RequestModel): Promise<any> {
    try {
      const sql =
        "CALL UpdateRequest(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, @err_code, @err_msg)";
      await this.db.query(sql, [
        req.request_id,
        req.subject,
        req.content,
  ...
  async deleteRequest(request_id: number): Promise<any> {
    try {
      ...
      await this.db.query(sql, [request_id]);
      return true;
    } catch (error: any) {
      throw new DatabaseError(error.message);
    }
  ...
  async getRequest(request_id: number): Promise<any> {
    try {
      ...
      const [results] = await this.db.query(sql, [request_id]);
      return results[0];
    } catch (error: any) {
      throw new DatabaseError(error.message);
    }
