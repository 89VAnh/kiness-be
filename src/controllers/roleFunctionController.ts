...
export class RoleFunctionController {
  ...
  async createRoleFunction(req: Request, res: Response): Promise<void> {
    try {
      ...
      await this.rfService.createRoleFunction(role.role_function_list, role.created_by_user_id);
  ...
  async deleteRoleFunction(req: Request, res: Response): Promise<void> {
    try {
      ...
      await this.rfService.deleteRoleFunction(object.list_json, object.updated_by_id);
